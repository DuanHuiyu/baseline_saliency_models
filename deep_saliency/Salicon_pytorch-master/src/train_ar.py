...
from src.data_utils_ar import getTrainVal_loader
from src.salicon_model import Salicon


def train_model(model,device,dataloaders,criterion,optimizer,num_epochs,check_point=1,model_save_name=''):
    model.train()
    ...
    for epoch in range(num_epochs):
        ...
        if num_epochs%check_point==0:
            torch.save(model.state_dict(),'salicon_model'+model_save_name+'.pth')
...
def main():
    ...
    dataloaders=getTrainVal_loader(args.train_dataset_dir,args.test_dataset_dir,args.train_img_dir,args.train_label_dir,data_csv=args.train_data_csv)
    ...
    # train model
    train_model(model_ft, device, dataloaders, criterion_ft, optimizer_ft, num_epochs=args.epochs, check_point=args.checkpoint, model_save_name=args.model_name)
