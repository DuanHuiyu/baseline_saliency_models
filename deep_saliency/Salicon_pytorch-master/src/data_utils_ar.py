...
class SaliconDataset(Dataset):
    ...
    def __init__(self,dataset_root,stimuli_dir,fixation_maps_dir,transform=None,use_cache=False,type='train',data_csv=None):
    ...
    def __getitem__(self, index):
        ...
        label_add=os.path.join(self.dataset_root,self.fixation_maps_dir,self.image_names[index].replace('.jpg','.png'))
        self.fine_coarse_add=fine_coarse_add
        original_img=Image.open(fine_coarse_add).convert('RGB')
        ...
        return [fine_img,coarse_img],label,self.image_names[index].replace('.jpg','.png')


    def __len__(self):
        return self.lens

def getTrainVal_loader(train_dataset_dir,test_dataset_dir,img_dir,label_dir,shuffle=True,val_split=0.1,data_csv=None):
    ...
    trainval_dataset = SaliconDataset(train_dataset_dir, img_dir, label_dir, transform=data_transforms, data_csv=data_csv)
    test_dataset = SaliconDataset(test_dataset_dir, img_dir, label_dir, transform=data_transforms, data_csv=data_csv)
    ...
    # val_loader=DataLoader(test_dataset,batch_size=1,sampler=valid_sampler,num_workers=6)
    train_loader=DataLoader(trainval_dataset,batch_size=1,num_workers=6)
    # val_loader=DataLoader(trainval_dataset,batch_size=1,sampler=valid_sampler,num_workers=6)
    val_loader=DataLoader(test_dataset,batch_size=1,num_workers=6)

    trainval_loaders={'train':train_loader,'val':val_loader}

    return trainval_loaders

def getTest_loader(test_dataset_dir,img_dir,label_dir,shuffle=True,data_csv=None):
